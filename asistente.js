...
(function () {
  ...
  function askWhen() {
    say('¿Para cuándo te acomoda?', function () {
      ['Lo antes posible', 'Esta semana', 'Fin de semana', 'Todavía no sé'].forEach(function (w) {
        chip(w, function () { st.when = w; me(w); finish(); });
      });
    });
  }
  function finish() {
    var que = st.det ? st.det.toLowerCase() + ' (' + st.svc.toLowerCase() + ')' : st.svc.toLowerCase();
    var cuando = { 'Lo antes posible': 'Me acomoda lo antes posible.', 'Esta semana': 'Me acomoda esta semana.', 'Fin de semana': 'Me acomoda el fin de semana.', 'Todavía no sé': 'Aún no tengo fecha definida.' }[st.when];
